...
function Header() {
    ...
    return (
        <div className="px-5 py-4  lg:px-15 xl:py-4 xl:gap-4 flex items-center justify-between">
            ...
            <div className="hidden lg:flex items-center gap-8">
                <div>
                    <ul className="flex gap-2 xl:gap-10 px-4 xl:px-[54.5px] py-6 border rounded-[50px] border-gray">
                        {LINKS.map((item) => (
                            <Link to={item.href} key={item.id} className="text-[12px] xl:text-base font-medium font-montserrat leading-[100%]"><li>{item.title}</li></Link>
                        ))}
                    </ul>
                </div>
            ...
            <CiMenuBurger onClick={handleMenu} className="flex lg:hidden cursor-pointer" />
            <ul className={`w-[95%] py-2 flex flex-col gap-3 rounded-2xl bg-white items-center justify-center shadow-[0_10px_30px_rgba(238,238,238,0.502)]  absolute left-1/2 -translate-x-1/2 top-18
                 transition-all duration-300 ease-out ${show ? "opacity-100 scale-100 translate-y-0 visible" : "opacity-0 scale-95 -translate-y-4 invisible"}`}>
                {LINKS.map((item) => (
